...
Unix 使用 I/O stream 读写数据，stream 是一个很灵活的概念 - 文件 / 设备 / 终端 / 其它 stream。如果使用 `cat` 时不加文件名，`cat` 将会继续执行，默认从 kernel 的标准输入获取参数，内核将标准输入连接到 terminal。
